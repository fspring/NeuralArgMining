...
def decodeTags(all_texts_tags):
    number_of_texts = len(all_texts_tags)
    all_texts_new_tags = []
    for i in range(0, number_of_texts):
        text_size = len(all_texts_tags[i])
        new_tags = []
        for j in range(0, text_size):
            arg = np.argmax(all_texts_tags[i][j]) + 1
            new_tags.append(arg)
        all_texts_new_tags.append(new_tags)
    return all_texts_new_tags

def replace_predicted_argument_tag(all_texts_tags):
    number_of_texts = len(all_texts_tags)
    for i in range(0, number_of_texts):
        text_size = len(all_texts_tags[i])
        is_premise = False
        is_claim = False
        for j in range(0, text_size):
            arg = all_texts_tags[i][j]
            if arg == 1:
                if is_premise:
                    all_texts_tags[i][j] = '(P)'
                elif is_claim:
                    all_texts_tags[i][j] = '(C)'
            elif arg == 2:
                is_premise = False
                is_claim = False
                all_texts_tags[i][j] = '(O)'
            elif arg == 3:
                is_premise = True
                is_claim = False
                all_texts_tags[i][j] = '(P)'
